...
parser.add_argument('--synpg_model_path', type=str, default="./model_new/rte-bert40.pt",#"./model_finetune/rte_epoch-bert50.pt",#"./model_new/rte-distillbert40.pt",#"./model_finetune/mrpcfull_epoch50.pt",##"./finetune_full/synpg_epoch-140.pt",#"./model/pretrained_synpg.pt",#"model_finetune/synpg_epoch100020.pt",#"finetune_full/synpg_epoch-135.pt",#"./mrpc_finetune/synpg_epoch240.pt", #"./model/pretrained_synpg.pt", 
                       help="prtrained SynPG")
parser.add_argument('--pg_model_path', type=str, default="./model_parse/parse_rte_epoch40.pt",#
                       help="prtrained parse generator")
parser.add_argument('--train_data_path', type=str, default="./data/rte_train.h5",
                       help="input file")
parser.add_argument('--output_path', type=str, default="./RTE1-after/",
                       help="output file")
parser.add_argument('--output_path1', type=str, default="./RTE-out-after/",
                       help="output file")
...
parser.add_argument('--max_sent_len', type=int, default=135,
                       help="max length of sentences")
parser.add_argument('--max_tmpl_len', type=int, default=100,
                       help="max length of tempalte")
parser.add_argument('--max_synt_len', type=int, default=342,
...
#tokenizer = AutoTokenizer.from_pretrained("textattack/roberta-base-RTE",cache_dir = './huggingface/token')#.cuda()
#modelclass = AutoModelForSequenceClassification.from_pretrained("textattack/roberta-base-RTE",cache_dir = './huggingface/model')#.cuda()

#tokenizer = AutoTokenizer.from_pretrained("textattack/distilbert-base-uncased-RTE",cache_dir = './huggingface/token')#.cuda()
#modelclass = AutoModelForSequenceClassification.from_pretrained("textattack/distilbert-base-uncased-RTE",cache_dir = './huggingface/model')#.cuda()
tokenizer = AutoTokenizer.from_pretrained("textattack/bert-base-uncased-RTE",cache_dir = './huggingface/token')#.cuda()
modelclass = AutoModelForSequenceClassification.from_pretrained("textattack/bert-base-uncased-RTE",cache_dir = './huggingface/model')#.cuda()

modelclass.load_state_dict(torch.load('./class/class4_rte01.pt'),strict=False)
...
def load_data(name):
    ...
    data = (h5f["label"],h5f["sent1"], h5f["sent2"],h5f["synt1"],h5f["synt2"])
    return data
...
templates = [
    "( ROOT ( S ( S ( NP ) ( VP ) ) ( , , ) ( CC but ) ( S ( NP ) ( VP ) ) ( . . ) ) )",
    "( ROOT ( S ( NP ( NP ) ( PP ) ) ( VP ( VBP ) ( VP ) ) ( . . ) ) )",
    "( ROOT ( S ( NP ( NP ) ( , ) ( NP ) ( , ) ) ( VP ( VBZ ) ( VP ) ) ( . . ) ) )",
    "( ROOT ( S ( S ( NP ) ( VP ) ) ( , , ) ( CC and ) ( S ( NP ) ( VP ) ) ( . . ) ) )",
    "( ROOT ( S ( S ( NP ) ( VP ) ) ( CC and ) ( S ( NP ) ( VP ) ) ( . . ) ) )",
]
...
with open(os.path.join(args.output_path, f"output_my_bertattack01-parse.txt"), "w") as fp1, \
    open(os.path.join(args.output_path, f"output_my_bertsen01-parse.txt"), "w") as fp2,\
    open(os.path.join(args.output_path1, f"output_my_rte-bert01.txt"), "w") as fp3:
    ...
    for it, data_idxs in enumerate(train_loader):
        ...
        for i in range(batch_size):
            ...
            for j in range(len(tmpls)):
                ...
                results = tokenizer(sent2,paraphrases[j], return_tensors="pt")
                ...
                if int(result)!=int(label):
                    ...
                    fp2.write(sent+'\n')
                    fp3.write(str(label)+'	'+paraphrases[j]+ '	'+sent2+'\n')
            if att!=0:
                attack+=1
                print('attack')
